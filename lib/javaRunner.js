...
const JDK_CACHE = path.join(process.cwd(), '.codemaster-jdk');
...
async function downloadBundledJdk(onProgress) {
  await fs.mkdir(JDK_CACHE, { recursive: true });
  const archivePath = path.join(JDK_CACHE, `jdk-${JDK_VERSION}.tar.gz`);
  ...
  const extractDir = path.join(JDK_CACHE, 'extracted');
  ...
  await fs.writeFile(path.join(JDK_CACHE, 'java-home.txt'), home, 'utf8');
